...
A Django app should own a coherent set of related concepts and behaviours. It should not necessarily
correspond to a navigation item or an individual screen.
...
This prevents notes and whiteboards from becoming scattered across Academics, Collaboration and
Planning.
...
Other apps should generate notification events, but notifications should control how they are stored
and presented.
Academics
-> Assignment reminder
Planning
...
Search may initially live in core, but a dedicated app is appropriate once global search becomes a major
feature.
...
Assignment group membersacademics, linked to accounts
Notes content
Whiteboards content
Revision academics
...
Assignments belong to academics, but use task behaviour from planning.
Notes are owned by content and linked everywhere else.
Files are owned by files and linked everywhere else.
Group Projects belong to collaboration, but use tasks, notes and files from their owning apps.
The Dashboard belongs to core, but owns almost none of the data it displays.
Search and Notifications operate across all apps without owning the underlying feature data.
1.
2.
3.
4.
